...
function formatInvoiceNumber(number) { return String(number || 121).padStart(6, "0"); }

function newInvoice(number = 121) {
  const today = new Date(); const due = new Date(today); due.setDate(due.getDate() + 15);
  return { id: Date.now() + Math.random(), invoiceNo: formatInvoiceNumber(number), customer: "", customerPhone: "", archiveId: null, items: [], paymentMethod: "cash", paidAmount: 0, orderSource: "office", deliveryDate: "", dueDate: due.toISOString().slice(0,10), payments: [] };
}
...
const NEXT_INVOICE_KEY = "alwan-system-next-invoice-v1";
const EXPENSES_KEY = "alwan-system-expenses-v1";
const SUPPLIERS_KEY = "alwan-system-suppliers-v1";
...
function InvoiceScreen({ invoices, setInvoices, activeId, setActiveId, settings, onArchive, archiveStatus, onNewInvoice }) {
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-4 no-print">
        ...
        <Field label="رقم الفاتورة">
          <div className="flex gap-2">
            ...
            <span className="px-3 flex items-center rounded-lg border border-stone-300 text-stone-500 text-xs">تلقائي</span>
          </div>
        </Field>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Field label="طريقة الدفع"><Select value={active.paymentMethod || "cash"} onChange={(v)=>updateActive({paymentMethod:v})} options={[{value:"cash",label:"كاش"},{value:"wallet",label:"محفظة"},{value:"instapay",label:"إنستاباي"},{value:"credit",label:"آجل"}]}/></Field>
          <Field label="مصدر الأوردر"><Select value={active.orderSource || "office"} onChange={(v)=>updateActive({orderSource:v})} options={[{value:"office",label:"المكتب"},{value:"online",label:"أونلاين"}]}/></Field>
          <Field label="المدفوع الآن"><NumInput value={active.paidAmount || ""} onChange={(e)=>updateActive({paidAmount:Number(e.target.value)||0})}/></Field>
          <Field label="تاريخ استحقاق الدفع"><input type="date" value={active.dueDate || ""} onChange={(e)=>updateActive({dueDate:e.target.value})} className="w-full rounded-lg border px-3 py-2"/></Field>
        </div>
        <Field label="موعد التسليم"><input type="datetime-local" value={active.deliveryDate || ""} onChange={(e)=>updateActive({deliveryDate:e.target.value})} className="w-full rounded-lg border px-3 py-2"/></Field>
      ...
      <div className="print-area bg-white rounded-2xl border border-stone-200 p-5">
        <div className="flex items-center justify-between border-b border-stone-200 pb-4 mb-4">
          <div className="flex items-center gap-3">
            <img src={BRAND_LOGO} alt="مطبعة ألوان" className="w-20 h-16 object-contain" />
            <div>
            ...
            {active.items.map((it, itemIndex) => {
              ...
              return (
                <div key={it.id} className="grid grid-cols-12 gap-1 items-center py-2 border-b border-dashed border-stone-200">
                  <span className="col-span-6 text-stone-700 text-sm"><b className="block text-[10px] text-cyan-600">{it.itemCode || `${active.invoiceNo}-${itemIndex+1}`}</b>{it.description}</span>
        ...
        <div className="mt-2 text-xs text-stone-500 flex justify-between"><span>المدفوع: {egp(active.paidAmount || 0)} ج.م</span><span>المتبقي: {egp(Math.max(0,total-(active.paidAmount||0)))} ج.م</span></div>
      </div>
...
function DashboardScreen({ archivedInvoices, orders, production, canViewFinance, user }) {
  ...
  return <div className="space-y-5">
    ...
    {overdue.length>0&&<div className="rounded-2xl border border-orange-500/50 bg-orange-950/30 p-4"><div className="font-bold text-orange-300 mb-2">⚠ عملاء متأخرون عن السداد أكثر من 15 يومًا</div>{overdue.slice(0,6).map(inv=><div key={inv.id} className="flex justify-between text-sm py-2 border-b border-orange-900/40"><span>{inv.customer||"عميل"} — #{inv.invoiceNo}</span><b>{egp(invoiceTotal(inv)-((inv.payments||[]).reduce((s,p)=>s+Number(p.amount||0),0)||Number(inv.paidAmount||0)))} ج.م</b></div>)}</div>}
...
function ProductionScreen({ production, setProduction, user, addAudit }) {
  ...
  const stages=[{id:"inquiry",label:"تواصل / استفسار",owner:"محمد سعيد"},{id:"quote",label:"عرض سعر",owner:"محمد سعيد"},{id:"approved",label:"موافقة العميل",owner:"محمد سعيد"},{id:"design",label:"تصميم وتجهيز",owner:"محمد مصطفى"},{id:"design-approved",label:"اعتماد التصميم",owner:"محمد مصطفى"},{id:"execution",label:"تنفيذ / طباعة",owner:"عبد الرحمن عيد"},{id:"finishing",label:"تشطيب / خارجي",owner:"عمر عادل"},{id:"review",label:"مراجعة وتجهيز",owner:"عمر عادل"},{id:"delivery",label:"تسليم / شحن",owner:"عمر عادل"}];
  function add() { if (!title.trim()) return; const first=stages[0];const next = [{ id: uid("prod"), title: title.trim(), stage:first.id, assignee:first.owner, priority:"normal", createdAt: new Date().toISOString(), by: user.name }, ...production]; setProduction(next); saveKey(PRODUCTION_KEY, next); addAudit("إضافة أمر إنتاج", title.trim()); setTitle(""); }
  function update(id,patch) { const next=production.map(p=>p.id===id?{...p,...patch,updatedAt:new Date().toISOString()}:p);setProduction(next);saveKey(PRODUCTION_KEY,next);addAudit("تحديث أمر إنتاج",id); }
  return <div className="space-y-4"><div className="bg-white rounded-2xl border border-slate-200 p-4"><SectionHeader title="إضافة مهمة إنتاج" accent="bg-cyan-500" /><div className="flex gap-2"><TextInput value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="مثال: 000135-1 — طباعة كتاب" /><button onClick={add} className="shrink-0 bg-blue-600 text-white rounded-xl px-4">إضافة</button></div></div><div className="grid lg:grid-cols-3 gap-3">{production.map((p)=>{const stage=stages.find(s=>s.id===(p.stage||p.status))||stages[0];return <div key={p.id} className="bg-white rounded-2xl border border-slate-200 p-4"><div className="font-semibold text-slate-800">{p.title}</div><div className="text-[11px] text-slate-400 mt-1">{new Date(p.createdAt).toLocaleString("ar-EG")}</div><div className="mt-3 space-y-2"><Field label="المرحلة"><Select value={stage.id} onChange={v=>{const s=stages.find(x=>x.id===v);update(p.id,{stage:v,assignee:s.owner})}} options={stages.map(s=>({value:s.id,label:s.label}))}/></Field><Field label="المسؤول"><TextInput value={p.assignee||stage.owner} onChange={e=>update(p.id,{assignee:e.target.value})}/></Field></div></div>})}{!production.length&&<div className="text-center text-slate-400 bg-white rounded-2xl p-8">لا توجد مهام إنتاج.</div>}</div></div>;
}
...
function CustomersScreen({ invoices, onOpen }) {
  const [query,setQuery]=useState("");const [from,setFrom]=useState("");const [to,setTo]=useState("");
  const filtered=invoices.filter(i=>(!from||new Date(i.createdAt)>=new Date(from))&&(!to||new Date(i.createdAt)<=new Date(to+"T23:59:59")));
  const map={};filtered.forEach(inv=>{const key=inv.customerPhone||inv.customer||"بدون عميل";if(!map[key])map[key]={name:inv.customer||"بدون اسم",phone:inv.customerPhone||"",invoices:[],total:0,paid:0};map[key].invoices.push(inv);map[key].total+=invoiceTotal(inv);map[key].paid+=(inv.payments||[]).reduce((s,p)=>s+Number(p.amount||0),0)||Number(inv.paidAmount||0);});
  const customers=Object.values(map).filter(c=>!query||c.name.includes(query)||c.phone.includes(query));
  function whatsapp(c){const debt=c.total-c.paid;const text=`أ/ ${c.name}\nكشف حسابكم لدى مطبعة ألوان\nإجمالي الأعمال: ${egp(c.total)} ج.م\nالمدفوع: ${egp(c.paid)} ج.م\nالمتبقي: ${egp(debt)} ج.م`;window.open(`https://wa.me/2${c.phone.replace(/\D/g,"").replace(/^0/,"")}?text=${encodeURIComponent(text)}`,"_blank");}
  return <div className="space-y-4"><div className="bg-white rounded-2xl border border-slate-200 p-4"><SectionHeader title="بحث وكشف حساب العملاء" accent="bg-blue-600"/><div className="grid md:grid-cols-3 gap-3"><Field label="الاسم أو الهاتف"><TextInput value={query} onChange={e=>setQuery(e.target.value)} placeholder="ابحث..."/></Field><Field label="من"><input type="date" value={from} onChange={e=>setFrom(e.target.value)} className="w-full rounded-lg border px-3 py-2"/></Field><Field label="إلى"><input type="date" value={to} onChange={e=>setTo(e.target.value)} className="w-full rounded-lg border px-3 py-2"/></Field></div></div>{customers.map(c=>{const debt=c.total-c.paid;return <div key={c.phone+c.name} className="bg-white rounded-2xl border border-slate-200 p-4"><div className="flex justify-between gap-3"><div><b>{c.name}</b><div className="text-xs text-slate-400">{c.phone} — {c.invoices.length} فاتورة</div></div><div className="text-left"><div className="text-xs text-slate-400">عليه</div><b className={debt>0?"text-red-400":"text-emerald-400"}>{egp(debt)} ج.م</b></div></div><div className="grid grid-cols-3 gap-2 mt-3 text-xs"><div>أعمال: {egp(c.total)}</div><div>مدفوع: {egp(c.paid)}</div><div>متبقي: {egp(debt)}</div></div><div className="flex gap-3 mt-3"><button onClick={()=>whatsapp(c)} className="text-emerald-400 text-xs">إرسال واتساب</button>{c.invoices[0]&&<button onClick={()=>onOpen(c.invoices[0])} className="text-blue-400 text-xs">فتح آخر فاتورة</button>}</div></div>})}</div>;
}

function ExpensesScreen({ expenses,setExpenses,addAudit,entryOnly=false }){const blank={category:"petty",amount:"",note:"",date:new Date().toISOString().slice(0,10),method:"cash"};const[form,setForm]=useState(blank);function add(){if(!Number(form.amount))return;const next=[{...form,id:uid("exp"),amount:Number(form.amount),createdAt:new Date().toISOString()},...expenses];setExpenses(next);saveKey(EXPENSES_KEY,next);addAudit("تسجيل مصروف",`${form.amount} - ${form.note}`);setForm(blank);}const cats=[{value:"petty",label:"نثريات"},{value:"materials",label:"خامات"},{value:"bills",label:"فواتير"},{value:"rent",label:"إيجار"},{value:"maintenance",label:"صيانة"},{value:"stationery",label:"أدوات مكتبية"},{value:"external",label:"شغل خارجي"},{value:"other",label:"أخرى"}];return <div className="space-y-4"><div className="bg-white rounded-2xl border border-slate-200 p-4"><SectionHeader title="تسجيل مصروف" accent="bg-red-500"/><div className="grid grid-cols-2 md:grid-cols-5 gap-3"><Field label="النوع"><Select value={form.category} onChange={v=>setForm({...form,category:v})} options={cats}/></Field><Field label="المبلغ"><NumInput value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})}/></Field><Field label="التاريخ"><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} className="w-full rounded-lg border px-3 py-2"/></Field><Field label="طريقة الدفع"><Select value={form.method} onChange={v=>setForm({...form,method:v})} options={[{value:"cash",label:"كاش"},{value:"wallet",label:"محفظة"},{value:"instapay",label:"إنستاباي"}]}/></Field><Field label="البيان"><TextInput value={form.note} onChange={e=>setForm({...form,note:e.target.value})}/></Field></div><button onClick={add} className="bg-blue-600 rounded-xl px-5 py-2">حفظ المصروف</button></div>{!entryOnly&&expenses.map(e=><div key={e.id} className="bg-white rounded-xl border border-slate-200 p-3 flex justify-between"><span>{cats.find(c=>c.value===e.category)?.label} — {e.note}</span><b>{egp(e.amount)} ج.م</b></div>)}</div>}

function SuppliersScreen({suppliers,setSuppliers,expenses,setExpenses,addAudit}){const blank={name:"",phone:"",type:"materials",total:"",paid:"",note:""};const[f,setF]=useState(blank);function add(){if(!f.name||!Number(f.total))return;const total=Number(f.total),paid=Number(f.paid)||0;const record={...f,id:uid("supplier"),total,paid,createdAt:new Date().toISOString()};const next=[record,...suppliers];setSuppliers(next);saveKey(SUPPLIERS_KEY,next);if(paid>0){const exp={id:uid("exp"),category:f.type==="external"?"external":"materials",amount:paid,note:`دفعة للمورد ${f.name}`,date:new Date().toISOString().slice(0,10),method:"cash",supplierId:record.id,createdAt:new Date().toISOString()};const ex=[exp,...expenses];setExpenses(ex);saveKey(EXPENSES_KEY,ex);}addAudit("فاتورة مورد",f.name);setF(blank);}return <div className="space-y-4"><div className="bg-white rounded-2xl border border-slate-200 p-4"><SectionHeader title="إضافة فاتورة مورد" accent="bg-amber-500"/><div className="grid grid-cols-2 md:grid-cols-3 gap-3"><Field label="المورد"><TextInput value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></Field><Field label="الهاتف"><TextInput value={f.phone} onChange={e=>setF({...f,phone:e.target.value})}/></Field><Field label="النوع"><Select value={f.type} onChange={v=>setF({...f,type:v})} options={[{value:"materials",label:"خامات"},{value:"external",label:"شغل خارجي"},{value:"maintenance",label:"صيانة"}]}/></Field><Field label="الإجمالي"><NumInput value={f.total} onChange={e=>setF({...f,total:e.target.value})}/></Field><Field label="المدفوع"><NumInput value={f.paid} onChange={e=>setF({...f,paid:e.target.value})}/></Field><Field label="البيان"><TextInput value={f.note} onChange={e=>setF({...f,note:e.target.value})}/></Field></div><button onClick={add} className="bg-blue-600 rounded-xl px-5 py-2">حفظ فاتورة المورد</button></div>{suppliers.map(s=><div key={s.id} className="bg-white rounded-xl border border-slate-200 p-4 flex justify-between"><div><b>{s.name}</b><div className="text-xs text-slate-400">{s.phone} — {s.note}</div></div><div className="text-left text-xs"><div>الإجمالي {egp(s.total)}</div><b className="text-red-400">متبقي {egp(s.total-s.paid)}</b></div></div>)}</div>}
...
export default function App() {
  ...
  const [nextInvoiceNumber, setNextInvoiceNumber] = useState(121);
  ...
  useEffect(() => {
    (async () => {
      ...
      setNextInvoiceNumber(await loadKey(NEXT_INVOICE_KEY, 121));
      setExpenses(await loadKey(EXPENSES_KEY, []));
      setSuppliers(await loadKey(SUPPLIERS_KEY, []));
      setLoaded(true);
    })();
  }, []);

  useEffect(()=>{const link=document.querySelector("link[rel~='icon']")||document.createElement("link");link.rel="icon";link.href=BRAND_LOGO;document.head.appendChild(link);document.title="مطبعة ألوان — نظام الإدارة";},[]);
  ...
  function createInvoice() {
    const inv = newInvoice(nextInvoiceNumber);
    setInvoices((prev)=>[...prev,inv]); setActiveId(inv.id);
    setNextInvoiceNumber((n)=>{const next=n+1;saveKey(NEXT_INVOICE_KEY,next);return next;});
  }
  ...
  function handleArchiveInvoice(invoiceId) {
    ...
    const record = { ...inv, id: archiveId, archiveId, createdAt, updatedAt: nowIso, items: inv.items.map((item,index)=>({...item,itemCode:item.itemCode||`${inv.invoiceNo}-${index+1}`})), payments: inv.payments?.length ? inv.payments : (Number(inv.paidAmount)>0 ? [{id:uid("pay"),amount:Number(inv.paidAmount),method:inv.paymentMethod,date:nowIso}] : []) };
  ...
  function handleOpenArchived(record) {
    ...
    if (alreadyOpen) {
      ...
      const draft = { ...record, id: Date.now() + Math.random(), archiveId: record.id };
  ...
  const sections = [
    ...
    ...(canViewFinance ? [{ id: "accounts", label: "الحسابات", icon: "●", defaultTab: "customers", tabs: ["customers", "archive", "expenses", "suppliers", "reports"] }] : [{id:"entry",label:"إدخال المصروفات",icon:"＋",defaultTab:"expense_entry",tabs:["expense_entry"]}]),
  ...
  const tabLabels = {
    ...
    production: "لوحة ترتيب الإنتاج", orders: `طلبات الإنتاج (${orders.length})`, customers:"بحث وحسابات العملاء", archive: `الفواتير (${archivedInvoices.length})`, expenses:"المصروفات", suppliers:"الموردون", reports: "التقارير والأرباح", expense_entry:"تسجيل مصروف",
    ...
    <div dir="rtl" className="min-h-screen bg-[#050a19] text-slate-100" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      <aside className="no-print fixed right-0 top-0 bottom-0 z-30 w-64 bg-[#0b1327] border-l border-[#22304b] p-4 hidden lg:flex flex-col">
        ...
        <nav className="space-y-2">
          {sections.map((section) => <div key={section.id}><button onClick={()=>setTab(section.defaultTab)} className={`w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition ${activeSection.id===section.id ? "bg-blue-600 text-white shadow-lg shadow-blue-950/50" : "text-slate-300 hover:bg-[#17213a]"}`}><span className="w-6 text-center">{section.icon}</span><span>{section.label}</span></button>{activeSection.id===section.id&&section.tabs.length>1&&<div className="mr-7 mt-2 space-y-1 border-r border-blue-500/30 pr-3">{section.tabs.map(id=><button key={id} onClick={()=>setTab(id)} className={`block w-full text-right rounded-lg px-3 py-2 text-xs ${tab===id?"bg-[#17284d] text-blue-300":"text-slate-400 hover:text-white"}`}>{tabLabels[id]}</button>)}</div>}</div>)}
        </nav>
      ...
      <main className="dark-workspace lg:mr-64 px-4 lg:px-7 py-6 pb-10 max-w-[1500px]">
          <>
            ...
            {tab === "invoice" && <InvoiceScreen invoices={invoices} setInvoices={setInvoices} activeId={activeId} setActiveId={setActiveId} settings={settings} onArchive={handleArchiveInvoice} archiveStatus={archiveStatus} onNewInvoice={createInvoice} />}
            {tab === "customers" && canViewFinance && <CustomersScreen invoices={archivedInvoices} onOpen={handleOpenArchived}/>} 
            {tab === "expenses" && canViewFinance && <ExpensesScreen expenses={expenses} setExpenses={setExpenses} addAudit={addAudit}/>} 
