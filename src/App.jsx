...
export default function App() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-900" style={{ fontFamily: "'IBM Plex Sans Arabic', sans-serif" }}>
      <style>{`
      ...
      <div className="max-w-md mx-auto px-4 pt-6 pb-3 no-print">
        <div className="flex items-center gap-2 text-stone-400 mb-1"><CMYKDots /><span className="text-[11px] tracking-widest">مطبعة ألوان</span></div>
        <h1 className="text-stone-50 text-2xl font-bold">نظام التسعير</h1>
      </div>

      <div className="max-w-md mx-auto px-4 no-print">
        <div className="flex gap-1 overflow-x-auto pb-3 -mx-1 px-1">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className={"shrink-0 rounded-full px-4 py-2 text-sm font-medium transition whitespace-nowrap " + (tab === t.id ? "bg-cyan-400 text-slate-900" : "bg-slate-800 text-stone-300 hover:bg-slate-700")}>{t.label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 pb-10">
        {!loaded ? (
          <div className="text-stone-400 text-center py-20 text-sm no-print">بيحمّل الإعدادات...</div>
        ) : (
          <>
            ...
            {tab === "settings" && <SettingsScreen settings={settings} setSettings={setSettings} onSave={handleSave} saveState={saveState} />}
          </>
        )}
      </div>
    </div>
